...
//	os_printf("jshDelayMicroseconds %f\n", jshGetMillisecondsFromTime(microsec));
	if (0 < microsec) [NSThread sleepForTimeInterval:(NSTimeInterval)microsec * 0.000001];
...
JsSysTime jshGetTimeFromMilliseconds(JsVarFloat ms) {
//  os_printf("jshGetTimeFromMilliseconds %f\n", ms);
  return (JsSysTime)(ms * 1000.0);
}

JsVarFloat jshGetMillisecondsFromTime(JsSysTime time) {
//  os_printf("jshGetMillisecondsFromTime %f\n", (JsVarFloat)time / 1000.0);
  return (JsVarFloat)time / 1000.0;
}
static JsSysTime startTime = 0;
JsSysTime jshGetSystemTime() { // in us
	JsSysTime time = (JsSysTime)([NSDate timeIntervalSinceReferenceDate] * 1000000.0) - startTime;
	if (0 == startTime) { startTime = time; return 0; }
	return time;
}
